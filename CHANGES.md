...
- Settings 页 API 配置简化为单页面——不再使用五标签页导航（对话供应商/账号/API密钥/网关/模型配对），点"API配置"直达 Provider 管理。
...
**设置页 UX 修复（5 项）**
- API 配置：fal/replicate 加入 Provider 列表，清除配置同步清理关联 account
- 模型配对：修复 focus 黑框（--accent → --border-accent），严格 isPaired 判断
- 生成设置：OtherPage 新增 defaultRatio/defaultStyle/defaultResolution 配置入口
- 模型选择器：ChatPanel 容器 overflow:visible + zIndex:10 修复裁剪
- useChat：style 中英文翻译 + coerceTaskForProvider 防报错

**代码改进**
- ChatPanel media constraints 感知（ratio/resolution 根据 provider 自动调整）
- useChat appendStyleToPrompt（style 值 append 到生成 prompt）
- ModelPairingPage 使用 createProviderSelectionPatch 替代简单 patch

**清理**
- 删除过时文件 7 个（AGENTS/CLAUDE/PRD/SPEC/SECURITY-AUDIT/FIXES）
- 修复 CHANGES.md mojibake
...
**鍥剧墖 API 配置涓庝腑杞珯婕旂ず**
- 新鈥滀腑杞珯 / GPT Image 2鈥濅綔涓哄浘鐗?API 涓诲叆鍙ｏ紝榛樿浣跨敤 `gpt-image-2` 涓?OpenAI Images 鍏煎璺緞
- API 配置椤垫寜 Hermes Agent 鐨?Provider-first 妯″紡閲嶆帓锛氬厛涓嬫媺閫夋嫨渚涘簲鍟嗭紝鍐嶉€夋嫨璁よ瘉鏂瑰紡銆佸～鍐欏嚟璇併€佹媺鍙栧苟閫夋嫨模型
- API 配置椤垫柊澧炲凡淇濆瓨模型配置涓嬫媺锛屽彲鐩存帴鍒囨崲鍘嗗彶 Provider + 鍑瘉 + Base URL + Model 缁勫悎
- Provider 涓嬫媺鍙睍绀哄彲鐩存帴璋冪敤鐨?API 渚涘簲鍟嗭紱璁㈤槄/璧勬枡鍏ュ彛淇濈暀涓哄崱鐗囪鏄庡拰璐拱/鐧诲綍閾炬帴锛岄伩鍏嶈淇濆瓨涓轰笉鍙皟鐢ㄩ厤缃?- 鍥剧墖配置椤电畝鍖栦负 Provider銆佽璇佹柟寮忋€丄PI Key/Session Token銆佷腑杞珯 Base URL銆佹ā鍨嬩笌娴嬭瘯鐢熷浘锛涘畼鏂?Provider 鐨?Base URL 绉诲叆楂樼骇閫夐」
- 模型鍖烘柊澧炲埛鏂版寜閽紝鑷姩鎷夊彇澶辫触鎴栧垰鏀瑰畬配置鏃跺彲鎵嬪姩閲嶆柊鑾峰彇模型鍒楄〃
- 新鍥剧墖鐪熷疄鐢熸垚娴嬭瘯锛屼娇鐢ㄦ渶灏忔祴璇?prompt 杩斿洖缂╃暐鍥撅紝閬垮厤鍙潬模型鍒楄〃璇垽鍙敤鎬?- Chat / Image / Video 鎵€鏈夊凡配置 Provider 閮戒細灏濊瘯鑷姩鎷夊彇模型鍒楄〃锛涒€滄祴璇曡繛鎺モ€濈粺涓€灞曠ず模型鎷夊彇缁撴灉锛岃嚜瀹氫箟鍥剧墖/视频鍏ュ彛涓嶅啀闇€瑕佹墜鍔ㄧ粫杩?- 模型鎷夊彇澶嶇敤 Provider 鑷韩鐨?Bearer銆丠eader銆丵uery銆丼ession 涓庢棤閴存潈配置锛屼笉鍐嶅彧鎸変腑杞珯鎴?Bearer Key 澶勭悊
- 淇濆瓨鍚庣殑鑴辨晱鍑瘉銆侀潪 Bearer 閴存潈涓庢棤閴存潈 Provider 浼氬湪模型鎷夊彇銆佽繛鎺ユ祴璇曘€佺湡瀹炵敓鎴愬拰蹇嵎模型鍒囨崲涓娇鐢ㄥ悓涓€濂楀垽鏂紝閬垮厤设置椤垫樉绀哄彲鐢ㄤ絾鐢熸垚鏃惰璇嫤
- 鐪熷疄 Provider 璋冪敤涓庡仴搴锋鏌ョ幇鍦ㄤ篃鏀寔鑷畾涔?`authType/customAuth` 瑕嗙洊鍜屾棤閴存潈 Provider锛岄伩鍏嶆ā鍨嬭兘鎷変絾鐢熸垚鏃朵粛鎸?Bearer 鍙戦€?- 鑷畾涔夊浘鐗?视频 API 鐨勮璇佹柟寮忚ˉ榻?Query Key 閫夐」锛屽苟鍦?fallback Provider 鍒楄〃涓繚鐣欏畬鏁撮壌鏉冮€夐」
- 缂洪厤缃彁绀烘敼涓?Provider / 鍑瘉 / 模型锛屼笉鍐嶆妸鎵€鏈夐壌鏉冮兘鎻忚堪鎴?API Key锛涘揩鎹锋ā鍨嬫寜閽湪褰撳墠配置鏈尮閰嶅巻鍙?profile 鏃舵樉绀哄綋鍓嶆ā鍨嬶紝閬垮厤璇涓虹涓€鏉″巻鍙查厤缃?- 蹇嵎鍒囨崲淇濆瓨杩囩殑 Provider profile 鍚庯紝涓昏繘绋嬭皟鐢ㄤ細浼樺厛鍖归厤宸蹭繚瀛?profile 鍑瘉锛岄伩鍏嶄繚瀛樿惤鐩樺墠鐭殏璋冪敤鏃?Provider锛涘巻鍙?metadata/璁㈤槄 profile 涓嶅啀鍑虹幇鍦ㄥ揩鎹峰垏鎹㈡垨 saved profile 涓嬫媺涓?- 鍚屼竴 Provider 涓嬪垏鎹笉鍚?Base URL/模型 profile 鏃讹紝涓昏繘绋嬩篃浼氬尮閰嶅搴斿凡淇濆瓨 profile锛岄伩鍏嶅揩鍒囧悗鐭殏浣跨敤鏃х鐐规垨鏃фā鍨?- 设置页€夋嫨宸蹭繚瀛?profile 鍚庯紝鑷姩鎷夊彇模型涓庤繛鎺ユ祴璇曚篃浼氫粠瀵瑰簲 profile 鍥炲彇鑴辨晱鍑瘉锛屼笉鍐嶅彧渚濊禆褰撳墠 active provider
- Chat / Image / Video 鐨勬墍鏈夊彲鎵ц Provider 閮戒細鍦ㄥ綋鍓嶉厤缃尯鐩存帴灞曠ず Base URL 涓庢ā鍨嬫媺鍙栧叆鍙ｏ紝涓嶅啀鍙妸涓浆绔欏綋浣滃彲配置 API
- 设置椤典笉鍐嶇敤涓绘祦 Provider 鐧藉悕鍗曢殣钘?registry/fallback 涓殑鍏跺畠 API锛涚櫧鍚嶅崟鍙繚鐣欎负鎺掑簭鏉冮噸锛岄伩鍏嶆柊澧?Provider 宸叉敞鍐屽嵈鏃犳硶閫夋嫨
- 模型鎷夊彇浼氳瘑鍒畼鏂?API 宸插寘鍚増鏈矾寰勭殑 Base URL锛堜緥濡?`/v1`銆乣/v1beta`銆乣/api/v3`锛夛紝閬垮厤鎶婄伀灞辩瓑瀹樻柟绔偣閿欒鎷兼垚閲嶅鐗堟湰璺緞
- Gemini 模型鎷夊彇浼氳瘑鍒敤鎴峰～鍐欑殑 `/v1beta` 绔偣锛屼笉鍐嶉噸澶嶆嫾鎺?`/v1beta/v1beta/models`
- 模型配置鍦ㄤ笅鎷夐€夋嫨涔嬪濮嬬粓淇濈暀鎵嬪姩杈撳叆妗嗭紝鍦ㄧ嚎鍒楄〃鎴栧唴缃洰褰曟病鏈夎鐩栫殑鏂版ā鍨嬩篃鑳界洿鎺ュ～鍐?- 鑷姩/鎵嬪姩鎷夊彇模型鍚庝細鏄剧ず鎷夊彇缁撴灉銆佺┖鍒楄〃鎻愮ず鎴栧け璐ユ彁绀猴紝涓嶅啀闈欓粯娓呯┖模型鍒楄〃
- 设置椤典富鍔ㄦ媺鍙栨ā鍨嬫垨娴嬭瘯杩炴帴鏃朵細鏄剧ず Provider 杩斿洖鐨勮劚鏁忛敊璇紝涓嶅啀鎶?Key銆丅ase URL 鎴栫鐐归敊璇吉瑁呮垚绌烘ā鍨嬪垪琛?- Provider 涓嶆彁渚涙爣鍑嗘ā鍨嬪垪琛ㄦ帴鍙ｆ椂浼氶檷绾т负鎺ㄨ崘模型/鎵嬪姩杈撳叆鎻愮ず锛屼笉鍐嶆妸 404/405 璇垽涓哄嚟璇佷笉鍙敤
- Provider 榛樿模型涓虹┖浣嗗唴缃洰褰曟湁鎺ㄨ崘椤规椂锛岄€夋嫨 Provider 浼氳嚜鍔ㄥ啓鍏ョ涓€鏉℃帹鑽愭ā鍨嬶紝閬垮厤涓嬫媺鍙浣嗕繚瀛橀厤缃负绌?- 鏃ч厤缃垨淇濆瓨 profile 鏃朵篃浼氱敤 Provider 鎺ㄨ崘模型琛ラ綈绌烘ā鍨嬶紝閬垮厤閲嶅惎鍚庡揩鎹锋ā鍨嬪垏鎹㈡紡鎺夎配置
- 淇濆瓨设置鍓嶄細鍐嶆褰掍竴鍖栧綋鍓?Provider 鐨勭┖模型锛岄伩鍏嶇晫闈㈡樉绀烘帹鑽愭ā鍨嬩絾 active provider 钀界洏涓虹┖
- 鑷畾涔?API 澧炲姞模型鍒楄〃璺緞配置锛岃嚜鍔ㄦ媺鍙栨ā鍨嬪彲閫傞厤 `/api/models`銆乣/v1/model/list` 绛夐潪鏍囧噯绔偣锛屽苟闄愬埗涓虹浉瀵硅矾寰勯伩鍏嶇粫杩?Base URL
- 鑷畾涔夊浘鐗?视频 API 楂樼骇配置新璇锋眰鏂规硶涓?JSON Body 妯℃澘锛屽彲鐩存帴閫傞厤闈炴爣鍑嗙敓鎴愩€佹彁浜ゅ拰杞鎺ュ彛锛屼笉鍐嶉渶瑕佹敼浠ｇ爜
- 鏆傛棤鍘熺敓 handler 鐨勫浘鐗?视频 API Provider 鐜板湪鍙綔涓衡€滆嚜瀹氫箟 API鈥濋厤缃細閫夋嫨 Provider 鍚庡～鍐欓珮绾ц矾寰勪笌妯℃澘鍗冲彲璧伴€氱敤 custom pipeline
...
- 鍒囨崲 Provider 鏃朵細娓呯悊鏃?Provider 鐨勯珮绾фā鏉裤€佽矾寰勩€佹柟娉曘€丅ody銆佽疆璇笌模型鍒楄〃璺緞锛岄伩鍏嶆柊渚涘簲鍟嗚鐢ㄤ笂涓€濂?API 配置
- 鍒囨崲宸蹭繚瀛?Provider profile 涓庢竻绌洪厤缃篃澶嶇敤鍚屼竴濂楁竻鐞嗛€昏緫锛岄伩鍏嶅揩鎹锋ā鍨嬪垏鎹㈠悗娈嬬暀涓婁竴鏉?profile 鐨勯珮绾?API 瀛楁
- 鏃х増 profile 涓殑 `customTemplate`銆侀《灞傛ā鏉垮瓧娈靛拰 `modelsPath` 浼氬湪鍒囨崲/淇濆瓨鏃跺綊涓€鍖栧埌褰撳墠 `template/modelListPath` 缁撴瀯锛岄伩鍏嶅崌绾у悗涓㈠け鑷畾涔?API 配置
- 鎵嬪姩杈撳叆鎴栧垏鎹㈡ā鍨嬫椂涓嶄細鍐嶈Е鍙戣嚜鍔ㄦ媺鍙栵紝鑷姩鎷夊彇鍙搷搴?Provider銆佸嚟璇併€佺鐐瑰拰璁よ瘉鏂瑰紡鍙樺寲
- 蹇€熷垏鎹?Provider銆佸嚟璇佹垨绔偣鏃讹紝杈冩棭杩斿洖鐨勬ā鍨嬫媺鍙栬姹備細琚拷鐣ワ紝涓嶄細瑕嗙洊褰撳墠 Provider 鐨勬ā鍨嬪垪琛?- 褰撳嚟璇佹垨绔偣琚竻绌烘椂锛屼細绔嬪嵆鍙栨秷鏃фā鍨嬫媺鍙栫粨鏋滅殑鍥炲啓骞舵竻绌哄姞杞芥€侊紝閬垮厤鏃犳晥配置涓嬫樉绀烘棫模型
- 杩炴帴娴嬭瘯涓庢祴璇曠敓鍥句細浣跨敤褰撳墠閫夋嫨鐨勮璇佹柟寮忓拰鑷畾涔夎璇佸弬鏁帮紝涓嶅啀鍥為€€鍒?Provider 榛樿 authType
- 设置椤典繚瀛樹細绛夊緟涓昏繘绋嬮厤缃惤鐩樺畬鎴愬悗鍐嶅叧闂紝閬垮厤鍒氫繚瀛?Provider/模型鍚庣珛鍒荤敓鎴愭椂鍛戒腑鏃ч厤缃?- 设置淇濆瓨涓細绂佺敤 Escape/鍏抽棴鎿嶄綔锛屼繚瀛樺け璐ユ椂鍦ㄥ脊绐楀唴鏄剧ず閿欒锛屼笉鍐嶅彧鍐欏叆鎺у埗鍙?- Provider 鍋ュ悍妫€鏌ヤ細闄愬埗鑷畾涔?Header 鍚嶃€佹敮鎸?Query 閴存潈鍙傛暟锛屽苟鍦ㄧ敤鎴疯緭鍏ユ柊鏄庢枃鍑瘉鏃舵祴璇曞綋鍓嶈緭鍏ョ鐐癸紝鑰屼笉鏄敊璇洖钀藉埌榛樿绔偣
- 褰撳墠 Provider 配置鍖烘柊澧?API Key銆佹帶鍒跺彴銆佸厖鍊?璐拱涓庢枃妗ｇ洿杈炬寜閽紝鍑忓皯配置鏃跺湪鍗＄墖鍒楄〃閲屽弽澶嶅鎵惧叆鍙?- Provider 鍗＄墖涓庡綋鍓嶉厤缃尯新鈥滅洿杩?API / 鑷畾涔?API / 璁㈤槄璧勬枡 / 璧勬枡鍏ュ彛鈥濊皟鐢ㄧ姸鎬佸拰鎺ュ叆鏂瑰紡鎻愮ず锛屽尯鍒嗗彲璋冪敤 API 涓庝粎璐﹀彿/璁㈤槄璇存槑
- 褰撳墠 Provider 配置鍖烘柊澧炲氨缁鏌ワ紝閫愰」鏄剧ず鍑瘉銆佺鐐广€佹ā鍨嬫槸鍚﹂綈鍏紝骞跺彲涓€閿～鍏ユ帹鑽?Base URL 涓庢ā鍨?- 鑷姩鎷夊彇模型涓庢祴璇曡繛鎺ヤ細鍦ㄥ綋鍓?Base URL 涓虹┖鏃跺洖閫€鍒?Provider 榛樿绔偣锛屽噺灏戝垰閫変緵搴斿晢鍚庣殑鎵嬪姩配置姝ラ
- Provider 配置新鍐呯疆模型鐩綍锛氬湪绾挎媺鍙栧け璐ユ垨 Provider 涓嶆彁渚涙爣鍑嗘ā鍨嬫帴鍙ｆ椂锛屾ā鍨嬩笅鎷変粛浼氭樉绀烘帹鑽愭ā鍨嬶紝閬垮厤鐢ㄦ埛鎵嬪姩鏌ユā鍨嬪悕
- 娓叉煋绔?fallback Provider 鍒楄〃涔熶細淇濈暀璁よ瘉鏂瑰紡銆佹ā鍨嬬洰褰曘€佽皟鐢ㄧ姸鎬佸拰鎺ュ叆鏂瑰紡锛岄伩鍏嶄富杩涚▼ Provider 鍒楄〃鍔犺浇澶辫触鏃堕€€鍥炲埌鎵嬪伐配置
- 模型鍒楄〃杩斿洖寮傚父褰㈢姸鏃朵細琚畨鍏ㄨ涓虹┖鍒楄〃锛岄伩鍏嶈缃〉鍥犱负涓浆/Provider 杩斿洖鏍煎紡涓嶆爣鍑嗚€屽穿婧?- 鏈彇鍒版ā鍨嬪垪琛ㄤ笉鍐嶈褰撲綔 API 涓嶅彲鐢紱鐢ㄦ埛浠嶅彲鎵嬪姩杈撳叆模型锛屽苟鐢ㄦ祴璇曠敓鍥剧‘璁ょ湡瀹炵敓鎴愯兘鍔?- 鍥剧敓视频浠诲姟浼氶殢浠诲姟淇濆瓨鏉ユ簮鍥剧墖 URL锛屽苟鍦ㄦ墽琛屾椂鍥炴煡鐩爣对话璧勪骇锛岄伩鍏嶅垏鎹㈠璇濇垨寮傛鎵ц鏃朵涪澶辨潵婧愬浘
...
**鍒涗綔璋辩郴涓庢暟鎹煣鎬?*
- 新璧勪骇鐢熸垚璋辩郴锛氳褰?provider銆佹ā鍨嬨€乸rompt銆佸弬鏁般€佺埗璧勪骇銆佸弬鑰冭祫浜т笌浠诲姟鏉ユ簮
...
- 璧勪骇更新浼氭繁鍚堝苟 generation 瀛楁锛岄伩鍏嶅眬閮ㄥ洖鍐欏垎杈ㄧ巼銆佺姸鎬佹垨鍧愭爣鏃朵涪澶卞師 prompt銆佹ā鍨嬩笌鐖惰祫浜у叧绯?
- 瀵煎叆浼氳鑼冨寲娑堟伅涓庝换鍔＄粨鏋勶紝杩囨护闈炲璞℃秷鎭€佷慨澶嶉噸澶嶆秷鎭?ID锛屽苟鎶婂凡涓柇鐨勮繍琛屼腑浠诲姟鏍囪涓洪敊璇紝閬垮厤鏃ч」鐩鍏ュ悗姘镐箙杞湀
- 对话鏍囬鎺ㄦ柇浼氳烦杩囧潖娑堟伅鍜岀┖ user 鍐呭锛屼紭鍏堜娇鐢ㄧ涓€鏉℃湁鏁堢敤鎴锋彁绀猴紝閬垮厤瀵煎叆鍚庡嚭鐜扮┖鏍囬
...
- 对话娑堟伅銆佷换鍔＄姸鎬佷笌璧勪骇澧炲垹鏀圭殑璐︽湰閫昏緫鎶戒负绾嚱鏁帮紝骞剁撼鍏ユ牳蹇冩祴璇曡鐩?
- 对话璐︽湰宸ュ叿浼氬閿欓潪鏁扮粍 messages/assets锛岄伩鍏嶆棫鍧忔暟鎹Е鍙戝悓姝ュ穿婧?
- 新 `npm run test:core`锛岃鐩栬祫浜х粨鏋勫綊涓€鍖栥€佹棫椤圭洰瀵煎叆鍏煎涓庨敊璇彁绀烘牸寮?

**对话瀵煎叆/瀵煎嚭**
- 新褰撳墠对话瀵煎叆/瀵煎嚭 JSON锛屼繚鐣欐秷鎭€佽祫浜т笌鍒涗綔璋辩郴锛屽鍏ユ椂鍒涘缓鏂扮殑鏈湴对话鑰屼笉澶嶇敤澶栭儴 ID
- 瀵煎嚭对话鏃朵細灏介噺鎶婅繙绋嬪浘鐗?视频鍐呰仈涓?data URL锛岄檷浣庝复鏃堕摼鎺ヨ繃鏈熷鑷翠綔鍝佷涪澶辩殑椋庨櫓
- 新椤圭洰绾у鍑?瀵煎叆锛屽彲涓€娆¤縼绉诲叏閮ㄥ璇濓紱瀵煎叆椤圭洰鏃朵互新璇濆悎骞讹紝涓嶈鐩栨湰鍦板巻鍙?
...
**鍒涗綔妗ｆ涓庤祫浜ф搷浣?*
- 鍒涗綔妗ｆ新鐢熸垚鏂瑰紡瀛楁锛屽彲鐩存帴鏌ョ湅 text-to-image銆乮mage-to-video 绛夌湡瀹?generation mode
- 璧勪骇璇︽儏鍗囩骇涓哄垱浣滄。妗堬紝鏀寔澶嶅埗 Prompt銆佸悓绯诲垪鍙樹綋銆佹崲椋庢牸銆侀噸鏂扮敓鎴愪笌鍥剧墖杞棰?
- 閲嶆柊鐢熸垚鍥剧墖淇濈暀鍘熷鎻愮ず璇嶄笉鍙橈紝缁曡繃 LLM 鐩存帴璋冪敤图像 API
...
- 鏍囪绱犳潗銆佸垹闄よ祫浜т笌鑷敱鐢诲竷鎷栧姩浣嶇疆浼氬嵆鏃跺悓姝ュ埌对话瀛樺偍锛屽噺灏戝揩閫熷垏鎹?瀵煎嚭鏃剁殑鐘舵€佽惤鍚?

**视频鐢熸垚澧炲己**
- 鍥剧墖杞棰戜細寮哄埗鎶婂綋鍓嶅浘鐗囦綔涓鸿棰?source image锛涜棰戣祫浜т笉鍐嶆彁渚涚粫杩囪垂鐢ㄧ‘璁ょ殑閲嶆柊鐢熸垚鎹峰緞
- 浠庡浘鐗囨墽琛屸€濆仛鎴愯棰戔€濅細鑷姩鍒囧埌视频宸ヤ綔鍖猴紝璁╀换鍔￠槦鍒楀拰瀹屾垚缁撴灉淇濇寔鍙

**鐢诲竷浜や簰鍗囩骇**
- 鑷敱鐢诲竷新璧勪骇绾ф挙閿€/閲嶅仛銆佸皬鍦板浘瀹氫綅涓庤氨绯荤嚎寮€鍏筹紝鎷栧姩銆佸垹闄ゃ€佹爣璁扮礌鏉愮瓑鐢ㄦ埛鎿嶄綔鍙仮澶?
- 新鏈湴 Agent 鍔ㄤ綔闃熷垪锛氬熀浜庨€変腑璧勪骇寤鸿涓嬩竴姝ュ姩浣滐紝鐢ㄦ埛纭鍚庡鐢ㄧ幇鏈夎祫浜у姩浣滄墽琛?
- Agent 闃熷垪鏀寔澶嶅埗鍙鏌ュ姩浣滆鍒掞紝骞跺湪鍒囨崲璧勪骇鏃舵竻鐞嗚繃鏈熷姩浣滐紝閬垮厤璇墽琛屾棫璧勪骇璁″垝

**鍥介檯鍖栦笌设置**
- 閲嶆柊鐢熸垚銆佺紪杈?Prompt銆佸浘鐗囪浆视频涓?Provider 棰勬閿欒浼氳窡闅忓綋鍓嶈瑷€鏄剧ず锛屽噺灏戣嫳鏂囩晫闈腑鐨勪腑鏂囩‖缂栫爜
- API 配置瀛楁锛圞ey/URL/Model锛夌Щ鑷?Provider 鍗＄墖缃戞牸涓婃柟锛屾棤闇€婊氬埌搴曢儴
- 璁㈤槄/濂楅绫?Provider 鐐瑰嚮鏄剧ず璧勬枡鍗＄墖锛堝畼缃?鏂囨。/璐拱鍏ュ彛锛夛紝涓嶈鍐欏叆璋冪敤配置
- 娓呯悊杩囨椂鐨勭粺涓€ API 页潰浠ｇ爜涓庨噸澶?Provider 閫夋嫨鍏ュ彛

#### v1.9.0 (2026-06-29)

**宸ヤ綔鍖轰笌模型鍏ュ彛閲嶆瀯**
- 榛樿杩涘叆鐢熷浘宸ヤ綔鍖猴紝鐙珛对话鍏ュ彛绉婚櫎锛屼絾淇濈暀鍏变韩对话闈㈡澘涓庡对话鍘嗗彶
- 模型閫夋嫨鍏ュ彛绉诲叆对话妗嗗伐鍏锋潯锛屽彧鏄剧ず设置涓繚瀛樿繃鐨勫璇濇ā鍨嬩笌褰撳墠濯掍綋模型
- 鐢诲竷鎸夊綋鍓嶅伐浣滃尯鑷姩杩囨护鍥剧墖/视频璧勪骇锛岀Щ闄ら噸澶嶇殑鍏ㄩ儴/鍥剧墖/视频绛涢€?
- OpenNana 鎻愮ず璇嶅簱鍏ュ彛绉诲埌鐢诲竷缃戞牸/鑷敱鍒囨崲鏃?

**API 设置涓?Provider 姊崇悊**
- 设置椤?API 配置鏀逛负对话/图像/视频涓夋爮鐩紝视频鏍忕洰璺熼殢瀹為獙视频寮€鍏虫樉绀?
...
- 视频鐢熸垚榛樿闅愯棌涓哄疄楠屽姛鑳斤紝闄嶄綆楂樻垚鏈鐢ㄩ闄?

#### v1.8.0 (2026-06-25)

**Provider Profiles 鈥?澶氶厤缃垏鎹?*
- 新 Provider Profiles 绯荤粺锛氭瘡涓建閬擄紙chat/image/video锛夊彲淇濆瓨澶氱粍 API Key + 模型缁勫悎
- 新 `ModelSelector` 缁勪欢鏇夸唬鍘?`ModelBar`锛氬湪鑱婂ぉ闈㈡澘椤堕儴蹇€熷垏鎹㈠凡淇濆瓨鐨勬ā鍨嬮厤缃?
- 配置鍔犲瘑瑕嗙洊 Profile 涓殑 API Key锛坄safeStorage` 鍔犲瘑锛?
- 设置椤靛ぇ骞呮墿灞曪細Profile 绠＄悊銆佹ā鍨嬫祴璇曘€佹墜鍔ㄨ緭鍏ユā鍨嬨€佹瘮渚?鍒嗚鲸鐜囬瑙?
- 鍒犻櫎模型鍘婚噸锛氶噸澶嶄繚瀛樺悓涓€ Provider+Model 缁勫悎鑷姩鍚堝苟

**UI 璋冩暣**
- 渚ф爮绮剧畝锛氱Щ闄?对话"妯″潡鍏ュ彛锛堣亰澶╅潰鏉垮缁堝彲瑙侊級锛岄粯璁よ繘鍏?鐢熷浘"
- 鐗堟湰鍙锋爣绛剧Щ鑷虫爣棰樻爮
- 鑱婂ぉ闈㈡澘鏍规嵁褰撳墠妯″潡锛堢敓鍥?视频锛夋樉绀哄搴旇緭鍏ユ彁绀?

#### v1.7.0 (2026-06-24)

**鍥藉唴濯掍綋 Provider 鎵╁睍**
- 新闃块噷涓囩浉 / Wan锛氱敓鍥撅紙wan2.6-t2i锛? 鐢熻棰戯紙wan2.7-t2v锛夛紝寮傛浠诲姟 submit/poll handler
- 新鐧惧害鍗冨竼锛氱敓鍥撅紙qwen-image锛? 鐢熻棰戯紙qianfan-video-latest锛夛紝寮傛浠诲姟 handler
- 新鑵捐娣峰厓 / TokenHub锛氱敓视频锛坔y-video-1.5锛夛紝OpenAI 鍏煎 submit/query handler
- 新 Vidu metadata 鍏ュ彛
- 新鐏北鏂硅垷 Coding Plan / OpenCode 璧勬枡鍏ュ彛
- Custom Image/Video 新鍗冨竼銆佹贩鍏冦€佷竾鐩搞€乂idu 涓浆棰勮
...
- 设置椤垫柊澧?Coding Plan / OpenCode / 鍗虫ⅵ 閾炬帴按钮
- 鍥藉唴 Provider 鑷姩鎺掑簭缃《

**Provider 娉ㄥ唽琛ㄥ畬鍠?*
- 鐏北鏂硅垷閾炬帴鍏ㄩ潰更新涓轰腑鏂囨枃妗?
- 闃块噷涓囩浉 integrationStatus 浠?metadata 鈫?handler
- 鍚?Provider 琛ュ厖中文鍚嶇О鍜屽浗鍐呭弸濂介摼鎺?
...
- Fallback Provider 鍒楄〃鍚屾锛欳hat 浠?9 涓墿灞曞埌 15 涓紝Image 新 SiliconFlow
...
- 新鍥藉唴骞冲彴鏀寔锛氱鍩烘祦鍔ㄣ€侀浂涓€涓囩墿銆丟roq銆乀ogether AI銆亁AI銆丳erplexity
...
- **UI 鏍峰紡瑙勮寖缁熶竴**锛氬鍔犱簡閬僵鑳屾櫙鑹层€佹笎鍙樹笌鍗遍櫓杈规鐨?CSS 鍏ㄥ眬鍙橀噺锛岀Щ杩囦簡涓昏缁勪欢涓殑纭紪鐮侀鑹蹭笌杈规设置锛屽叏闈㈡浛鎹负涓婚鍙橀噺銆?
- **鍥炬爣鍖呰鍣ㄧ粺涓€**锛氬湪 `icons.jsx` 涓鍔犱簡缂哄け绐楀彛鎺т欢涓庣敾甯冨伐鍏峰浘鏍囩殑鏄犲皠锛屽皢鍚勭粍浠朵腑鎵€鏈夊師鐢?`<svg>` 鍜?direct lucide 寮曠敤鍏ㄩ儴瑙勮寖鍖栨浛鎹负 `<Ic />`銆?

**鐘舵€佷笌鐢熷懡鍛ㄦ湡淇**
- **Ref 鐘舵€佸壇浣滅敤娓呯悊**锛氱Щ闄や簡 `AssetDetail.jsx` 鐘舵€佹洿新櫒鍐呴儴鐩存帴淇敼 ref 鐨勬搷浣滐紝寮曞叆浜嗕笌鍏跺悓姝ョ殑 `offsetRef` 璇诲彇鏈€新亸绉婚噺锛屼繚璇佷簡 React 鐘舵€佺殑绾害銆?
- **鍗＄墖鑷敱绉诲姩绔炴€佷慨澶?*锛氬湪 `useCanvas.js` 涓柊澧?`updateAssets` 鎵归噺更新鍑芥暟锛涢噸鏋勪簡 `CanvasPanel.jsx` 鍦?Free Mode 涓嬬殑鍧愭爣鍒濆鍖栵紝灏嗗潗鏍囧垎閰嶉噰鐢ㄥ崟娆″師瀛愭壒閲忔洿鏂帮紝褰诲簳娑堥櫎浜嗙敱浜庨€愪釜鍗＄墖淇敼瀵艰嚧鐨勯『娆￠噸娓叉煋绾ц仈鍜屾棤闄愬惊鐜殣鎮ｃ€?
- **浜嬩欢鐩戝惉娉勬紡淇**锛氬湪 `CanvasPanel.jsx` 涓紩鍏?`dragCleanupRef` 涓庡嵏杞芥竻鐞?effect锛屼繚璇佸湪缁勪欢鎰忓閿€姣佹椂閲婃斁缁戝畾鍦?window 涓婄殑鎷栨嫿鐩戝惉浜嬩欢锛屾潨缁濆唴瀛樻硠闇层€?
- **i18n 缂洪櫡淇涓庝紶鎾?*锛氫慨澶嶄簡 `MessageBubble.jsx` 鎺掗槦涓笌鏌ヨ视频鐘舵€佺殑涓嫳鏂囪浆鎹?bug锛屼娇 `ContextMenu.jsx` (鍙抽敭鑿滃崟) 涓?`TaskQueue.jsx` (浠诲姟闃熷垪) 鑳藉姝ｇ‘娑堣垂 `lang` 配置涓?`t()` 鍑芥暟缈昏瘧锛屽畬鎴愬簳鏍?model 杞ㄩ亾鐨勭炕璇戣浆鎹€?
...
- 娓叉煋杩涚▼配置鑴辨晱锛歚config:get` 鍙繑鍥?redacted API Key锛岀湡瀹炲瘑閽ヤ繚鐣欏湪涓昏繘绋嬪苟鐢?API IPC 璇诲彇
- 瀹夊叏淇濆瓨绱犳潗锛氱Щ闄ら珮椋庨櫓浠绘剰璺緞淇濆瓨鎺ュ彛锛屽浘鐗?视频淇濆瓨缁熶竴璧颁富杩涚▼鐢熸垚璺緞鎴栦繚瀛樺璇濇锛屽己鍒?`.png`/`.mp4` 鎵╁睍鍚?
- 涓嬭浇涓庡搷搴旈檺鍒讹細绱犳潗涓嬭浇澧炲姞 HTTPS 鏍￠獙銆乺edirect 澶嶆牎楠屻€?00MB 澶у皬涓婇檺銆佹€昏秴鏃跺拰涓存椂鏂囦欢 rename锛汚PI 鍝嶅簲澧炲姞 25MB 涓婇檺
- 鏂囦欢鍐欏叆鍘熷瓙鎬э細配置鍜屽璇濇暟鎹啓鍏ヤ娇鐢?tmp+rename 鍘熷瓙妯″紡锛屽苟鍙戝啓鎿嶄綔閫氳繃闃熷垪搴忓垪鍖?
- Gemini API Key URL 缂栫爜锛氫慨澶嶅惈鐗规畩瀛楃鏃?URL 鏂鐨勯棶棰?

**鐘舵€佺鐞嗕慨澶?*
- 对话閲嶅懡鍚嶆寔涔呭寲锛氫慨澶嶉噸鍛藉悕鍚庡埛鏂伴〉闈涪澶辩殑 bug
- 对话鍒囨崲闃蹭涪锛氬垏鎹?新缓/鍒犻櫎鍓?flush 褰撳墠对话锛屽紓姝?chat/image/video 缁撴灉鍐欏洖鍙戣捣对话鑰屼笉鏄涪寮冩垨涓插埌褰撳墠对话
- 对话瀛樺偍鎭㈠锛氭崯鍧忕殑 `conversations.json` 浼氬浠藉悗浠庣┖ store 鎭㈠锛屽垹闄?tombstone 鍦ㄨ鍙栧拰鍐欏叆鏃堕兘鐢熸晥
- 鐢诲竷鐘舵€佺ǔ瀹氭€э細useCanvas 杩斿洖鍊?memoization锛岄伩鍏嶇骇鑱旈噸娓叉煋
- 浠诲姟闃熷垪闂寘淇锛歶seTaskQueue 浣跨敤 canvasRef 娑堥櫎 stale closure
- 视频浠诲姟杞锛氳棰戞彁浜ゆ帴鍏ヤ换鍔￠槦鍒楋紝鎴愬姛浣嗘棤 `videoUrl` 鏃剁户缁?running锛屽畬鎴愬悗鐢熸垚 video asset
...
- 视频棰勮 CSP锛氬鍔?`media-src 'self' https: data: blob:`锛孉ssetCard/AssetDetail 浣跨敤 `<video controls>` 娓叉煋视频
...
**淇濆瓨寮圭獥淇**
- "淇濆瓨鍒版湰鍦?涓嶅啀寮瑰嚭涓ゆ对话妗嗭紝缁熶竴璧?IPC 閫氶亾
...
**对话鍛藉悕缂栬緫**
- 鍙屽嚮对话鏍囬鍙噸鍛藉悕锛屾敮鎸?Enter 纭 / Escape 鍙栨秷

**鍒嗚鲸鐜囨墿灞?*
- 新 2K (2560) 鍜?4K (3840) 鍒嗚鲸鐜囬€夐」锛屽昂瀵告寜姣斾緥鍔ㄦ€佺缉鏀?

#### v1.3.0 (2026-06-04)

**对话鐢熸垚设置**
- 对话杈撳叆鏍忔柊澧炪€岀敓鎴愯缃€嶉潰鏉匡紝鍙洿鎺ヨ皟鏁村浘鐗囨瘮渚嬨€侀鏍奸璁俱€佸垎杈ㄧ巼锛堟爣鍑?楂樻竻/瓒呮竻锛?
- 鐢熸垚设置浠?Settings 绉昏嚦对话妗嗭紝鏂逛究闅忔椂鍒囨崲锛屾棤闇€鎵撳紑设置椤?

**鎵归噺鐢熸垚**
- 鐢熸垚浠诲姟鍗＄墖新銆屾壒閲忋€嶆寜閽紝鏀寔涓€娆＄敓鎴?2/3/4 寮犲浘鐗?
- 鎵归噺杩涘害瀹炴椂鏄剧ず锛堝 2/4锛夛紝澶辫触椤硅烦杩囩户缁?

**璁℃椂鍣?*
- AI 鎬濊€冨拰鍥剧墖/视频鐢熸垚杩囩▼涓樉绀哄疄鏃惰鏃讹紙绉掓暟锛夛紝閬垮厤鐢ㄦ埛闀夸箙绛夊緟鏃犲弽棣?
- 鐢熸垚瀹屾垚鍚庢樉绀烘€荤敤鏃?

**API 鍙潬鎬т慨澶?*
- 淇鍒囨崲 Provider 鏃?`protocol` 瀛楁鏈繚瀛樺埌配置鐨?bug锛屽鑷村浘鐗囩敓鎴愬缁堝け璐?
- 新鍗虫ⅵ/Seedream (`ark_image`) 涓撶敤鍥剧墖鐢熸垚绔偣锛屼慨姝?URL 璺緞閿欒
- 鍥剧墖鐢熸垚新鑷姩閲嶈瘯鏈哄埗锛堝け璐ュ悗闂撮殧 2 绉掗噸璇?1 娆★級
- 视频鐢熸垚鍚屾牱淇 protocol 瑙ｆ瀽閫昏緫

**鐢诲竷妯″紡鍖哄垎**
- 缃戞牸妯″紡锛氱粨鏋勫寲鎺掑垪锛岃嚜鍔ㄥ垎鍒楋紝鏃犵缉鏀惧钩绉伙紝绾粴鍔ㄦ祻瑙?
- 鑷敱妯″紡锛氭棤闄愮敾甯冿紝鑷敱瀹氫綅锛? 鍒楅棿璺濇帓甯冿紝鏀寔缂╂斁骞崇Щ
- 新浘鐗?视频鐢熸垚鏃讹紝杈规閲戣壊鍛煎惛闂儊鍔ㄧ敾鎻愮ず鐢ㄦ埛

**鍙傝€冨浘功能鏀逛负鍙€?*
- 鍙傝€冨浘按钮榛樿闅愯棌锛屽湪设置 > 鍏朵粬涓彲寮€鍚?
- 寮€鍚悗对话妗嗗嚭鐜板弬鑰冨浘按钮
...
- 设置新銆岃嚜鍔ㄤ繚瀛樺浘鐗囧埌鏈湴銆嶅拰銆屽惎鐢ㄥ弬鑰冨浘銆嶅紑鍏?
...
**娣卞害鎬濊€?*
- 对话杈撳叆鏍忔柊澧炪€屾繁搴︽€濊€冦€嶅紑鍏筹紝寮€鍚悗璋冪敤 Anthropic extended thinking
- 鎬濊€冭繃绋嬪彲鎶樺彔灞曠ず锛岀嫭绔嬩簬姝ｆ枃鍐呭

**鍙傝€冨浘/视频**
- 对话杈撳叆鏍忔柊澧炪€屽弬鑰冨浘銆嶆寜閽紝鍙粠绱犳潗鐢诲粖閫夊彇澶氬紶鍙傝€冨浘
...
- 搴曢儴模型鏍忔寜閽姞澶э紝鏍囩澶у啓+閲戣壊鍒嗛殧绾匡紝鐗堟湰鍙疯兌鍥婃牱寮?
- 设置闈㈡澘杈撳叆妗嗗姞瀹斤紝淇濆瓨按钮娓愬彉+鎮仠涓婃诞
- 鍏ㄧ粍浠惰縼绉昏嚦 Lucide React 鍥炬爣搴?
- 鑷畾涔夊簲鐢ㄥ浘鏍?

**鍏朵粬**
- 淇对话鍒囨崲鏃跺唴瀹逛涪澶辩殑 bug锛坰tale closure + sync race condition锛?
...
**澶氬璇濈鐞?*
- 鏀寔澶氬璇濆苟琛岋紝姣忎釜对话鐙珛娑堟伅鍜岀敾甯冭祫浜?
- 对话鍒楄〃鏍忥細新缓銆佸垏鎹€佸垹闄ゅ璇?
- 对话鏁版嵁鑷姩鎸佷箙鍖栵紝鍒囨崲涓嶄涪澶?

**设置页潰閲嶆瀯**
- 宸︿晶瀵艰埅甯冨眬锛氶€氱敤设置锛堝瑙?璇█/鍏朵粬锛? API 配置锛堝璇?图像/视频锛?
- 模型瀛楁鑷姩鑾峰彇锛氳緭鍏?API Key 鍚庤嚜鍔ㄦ媺鍙栧彲鐢ㄦā鍨嬪垪琛紝涓嬫媺閫夋嫨
- Base URL 澧炲姞銆屾仮澶嶉粯璁ゃ€嶆寜閽?
- API 配置澧炲姞銆屾竻绌洪厤缃€嶆寜閽?
...
- 涓嫳鏂囧垏鎹紝设置椤?鏍囬鏍?搴曟爮/鑱婂ぉ闈㈡澘鏂囨璺熼殢璇█
- 瀛椾綋澶у皬鍙皟锛堝皬/涓?澶э級
- 设置缁勪欢鏀圭敤 CSS 鍙橀噺锛岃窡闅忎富棰樺垏鎹?

**浣撻獙鏀硅繘**
- 设置榻胯疆鍥炬爣鏇挎崲涓烘洿绮捐嚧鐨?Lucide Settings 鍥炬爣
- 娑堟伅姘旀场鏀寔鏂囨湰閫変腑澶嶅埗
- 对话杈撳叆妗?Shift+Enter 鎹㈣锛岃嚜鍔ㄥ楂?
- 鍥剧墖璧勪骇璇︽儏澧炲姞銆屼繚瀛樺埌鏈湴銆嶆寜閽?
- 鍥剧墖鐐瑰嚮鏀惧ぇ棰勮锛坙ightbox锛?
- 淇鍥剧墖/视频鐢熸垚澶辫触鏃舵棤鍙嶉鐨勯棶棰?
- 淇鎻忚堪鐢婚潰鏃惰瑙﹀彂鍥剧墖鐢熸垚鐨勯€昏緫闂
- 搴熷純模型鑷姩杩佺Щ锛堝鏃ч厤缃腑鐨?pollinations 鑷姩閲嶇疆锛?

#### v1.0.0 (2026-06-03)

- 对话椹卞姩鐨勫妯℃€佺敓鎴愶細杈撳叆鑷劧璇█锛孉I 鑷姩璇嗗埆鎰忓浘骞惰皟搴﹀浘鍍?视频浠诲姟
- 鏀寔澶氬对话銆佸浘鍍忋€佽棰?Provider
- 绱犳潗鐢诲粖鏀寔缃戞牸/鑷敱甯冨眬锛屽彸閿彍鍗曟搷浣?
- 视频鐢熸垚浠诲姟闃熷垪锛屾敮鎸佽繘搴﹁拷韪笌閲嶈瘯
- 设置闈㈡澘鎸夎建閬撶嫭绔嬮厤缃?Provider銆丄PI Key銆丅ase URL銆佹ā鍨?
