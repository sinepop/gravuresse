...
> 目标：定义支撑“对话 + Agent + 多 API + 稳定修改 + 多上下文 + Pipeline + Infinite Canvas + 可追溯复现”的核心架构，同时给出现有项目的渐进迁移路径。
...
v1.0 再把 Agent、Pipeline、Canvas、Lineage 合并为多视图系统
```
...
| Canvas layout | Canvas view store | 可写视图状态 |
...
### 3.8 CanvasDocument / PipelineDocument
...
Pipeline / Agent / Context / Replay 通过展开层进入。
...
7. Canvas 继续只是素材整理，Lineage 用详情页展示。
...
- Pipeline / Canvas / Lineage 三者边界清楚；
- 旧数据迁移可验证；
- 复现等级诚实；
